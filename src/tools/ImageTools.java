...
public class ImageTools {
    public static enum SimpleAction {
        // These three should not be used since they are way much slower then the java version.
     ...
     * <ul>
     *   <li> rotate 90 degrees right</li>
     *   <li> rotate 90 degrees left</li>
     *   <li> rotate 180 degrees</li>
     *   <li> mirror on the vertical axis</li>
     *   <li> mirror on the horizontal axis</li>
     *   <li> mirror on the diagonal (type 1) from the upper-left corner
     *    to the lower-right corner</li>
     *   <li> mirror on the diagonal (type 2) from the lower-left corner
     *    to the upper-right corner</li>
     * </ul>
     */
    public static BufferedImage simpleAction(BufferedImage biIn,
                                             SimpleAction rotationType) {
        ...
        if (rotationType == SimpleAction.ROTATE_90_RIGHT) {
            ...
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    newRgb[(height - 1 - y) * width + (width - 1 - x)]
                        = rgb[x + y * width];
                }
            }
            ...
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    newRgb[(height - 1 - y) + (width - 1 - x) * height]
                        = rgb[x + y * width];
                }
            }
     ...
     * @param type denotes the algorithm used. Must be one of:<ul>
     *       <li> {@link AffineTransformOp#TYPE_NEAREST_NEIGHBOR}</li>
     *       <li> {@link AffineTransformOp#TYPE_BILINEAR}</li>
     *       <li> {@link AffineTransformOp#TYPE_BICUBIC}</li>
     *     </ul>
     * @return a new {@link BufferedImage} containing the rotated image of biIn
     *     without corner-loss. The size of the image depends both on the size
     *     of the biIn and the angle.
     */
    ...
    /**
     * Converts an Image object to an BufferedImage object in a resource
     * efficient way.
     * 
     * @param img the image to be converted.
     * @return the converted image.
     * 
     * @see #toBufferedImage(java.awt.Image, int) &nbsp for more info on return value.
     */
     ...
     * 
     * @param img the image to be converted
     * @param type the new image type iff a new {@code BufferedImage} object
     *     is created.
     * @return <ul>
     *       <li> {@code null} if {@code img == null}</li>
     *       <li> {@code img} if {@code img instanceof} {@link BufferedImage}</li>
     *       <li> A new {@code BufferedImage} containing the image of {@code img}
     *            generated by {@link #imageDeepCopy(Image, int)}</li>
     *     </ul>
     * 
     * @see #imageDeepCopy(Image, int)
     */
     ...
     * @return a new {@link BufferedImage} from the source.
     */
    ...
    /**
     * Makes a deep copy of an image.
     * Use this to ensure that the given Image object is a new instance.
     * Note that {@code source != imageDeepCopy(source)} always holds.
     * 
     * @param source the image to be converted
     * @param type the type of the new BufferedImage
     * @return a new {@link BufferedImage} of the source
     */
    public static BufferedImage imageDeepCopy(Image source, int type) {
        if (source == null) return null;
        
        BufferedImage clone = new BufferedImage(source.getWidth(null),
